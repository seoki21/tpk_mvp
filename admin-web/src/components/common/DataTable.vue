...
<template>
  <div class="overflow-x-auto border border-gray-300 rounded">
    ...
    <div v-if="pageSize > 0 || total >= 0" class="flex items-center justify-between px-4 py-2 bg-gray-50 border-b border-gray-300">
      <!-- 좌측: 조회목록(총 N건) -->
      <span v-if="total >= 0" class="text-sm font-medium text-gray-700">
        {{ t('common.listTitle') }}({{ t('common.totalCount', { count: total }) }})
      </span>
      <span v-else></span>
      ...
      <thead class="bg-gray-100 border-b border-gray-300">
        <tr>
          <th
            ...
            :key="col.key"
            class="px-4 py-2 font-medium text-gray-700 select-none"
            :class="[
      ...
      <tbody>
        ...
        >
          <td
            ...
            :key="col.key"
            class="px-4 py-2"
            :class="col.align === 'left' ? 'text-left' : col.align === 'right' ? 'text-right' : 'text-center'"
          >
            <!-- 삭제여부 컬럼: N→공백, Y→빨간색 '삭제' -->
            <span
