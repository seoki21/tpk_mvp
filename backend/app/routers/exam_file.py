...
async def upload_files(exam_key: int, files: List[UploadFile] = File(...)):
    """
    ...
    try:
        ...
        rows = await exam_file_service.upload_files(exam_key, files)
...
@router.delete("/{pdf_key}", response_model=BaseResponse)
